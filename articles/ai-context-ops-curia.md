---
title: "Claude Code を複数プロジェクトで本気で使うための「AI文脈オーケストレーター」ツールを自作した"
emoji: "🗂️"
type: "tech"
...
![](/images/ai-context-ops-curia/project-context-claudecode-demo-2x.gif)
![](/images/ai-context-ops-curia/QuickCapture-2x.gif)
...
Curia はプロジェクトごとに `_ai-context/` フォルダを持ち、`current_focus.md` / `decision_log/` / `open_issues.md` などをMarkdownで管理します。Dashboard から全案件の状態を一望でき、どの案件に切り替えても、AIエージェントはその案件専用の文脈をそのまま読み込めます。人間側が毎回説明を組み直す必要がなくなり、切り替えコストが案件数に比例しなくなります。

### 2. LLM Wiki ― 知識ベースをLLM自身に育てさせる

Andrej Karpathy の「[LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)」の考え方を取り入れています。要点は、質問のたびに生データから再推論するのではなく、LLM が永続的な Wiki を育て続けること。Curia では取り込んだドキュメントを `sources` / `entities` / `concepts` に分解し、索引と更新履歴付きで蓄積していきます。案件ごとに「調べた内容」が再利用可能な資産として残ります。

### 3. ハーネスエンジニアリング ― Agent Skills とルールファイルで案件別にAIの振る舞いを切り替える

Curia はプロジェクトごとに `AGENTS.md` と Agent Skill ファイルを自動生成・管理します。Claude Code / Codex CLI は特別な設定なしに、そのままこのコンテキストファイル群を読み込みます。Agent Hub を使えば、サブエージェント定義やルールセットを案件別・CLI別に Deploy / Undeploy できるため、「案件Aのときはこの振る舞い、案件Bのときはあの振る舞い」という切り替えがUIから行えます。

やっていることはシンプルです。

- プロジェクトフォルダごとに `_ai-context/` を用意し、状況・意思決定・未解決事項をMarkdownで管理する
- AI補助機能でコンテキスト更新の手間を下げ、情報を常に新鮮な状態に保つ
- Claude Code / Codex CLI が特別な設定なしでコンテキストファイルをそのまま読み込める
- Agent Skill / サブエージェント定義を案件別に切り替えられる

結果として、複数プロジェクトを行き来しても、セッション開始時に「前回の続き」を説明し直す必要がなくなり、AIエージェントへの指示は常に「やりたいこと」だけに集中できるようになります。

なお、CuriaのEditorで更新した `current_focus.md` / `decision_log` / `open_issues.md` などのコンテキストファイルは、そのままClaude Codeから参照できます。`AGENTS.md` や Agent Skill 側にこれらのファイルを読み取る指示を入れており、`AGENTS.md` と Skill ファイルもCuriaが作成・管理します。
...
![](/images/ai-context-ops-curia/ImportMeetingNotes_1.png)
![](/images/ai-context-ops-curia/ImportMeetingNotes_2.png)
